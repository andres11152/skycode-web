...
export function SessionsView({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-background">Mi Cuenta</h1>
        <p className="mt-1 text-xs text-background/70 font-sans">
          Sesiones activas de {user.name} — cierre las que no reconozca
        </p>
      </div>

      <div className="rounded-xl border border-background/15 bg-background/5 p-5 space-y-1">
        <div className="text-xs text-background/60">Correo</div>
        <div className="text-sm font-mono text-background">{user.email}</div>
        <div className="mt-3 text-xs text-background/60">Rol</div>
        <div className="text-sm font-mono uppercase text-background/80">{user.role}</div>
      </div>

      {error && (
        <div className="rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-xs text-red-400">{error}</div>
      )}
                  ...
                  <tr key={s.id} className={isCurrent ? "bg-accent/5" : ""}>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <Monitor size={14} className="text-background/40 shrink-0" />
                    ...
                    <td className="px-5 py-3.5 font-mono text-background/60">
                      <span className="flex items-center gap-1">
                        <MapPin size={12} className="text-background/40" />
                        {s.ip || "—"}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 font-mono text-background/60 whitespace-nowrap">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-[11px] font-semibold text-red-400 hover:bg-red-500/20 transition-colors disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-foreground"
                      >
                        <LogOut size={12} />
