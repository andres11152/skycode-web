...
const PRIORITY_STYLES: Record<TicketPriority, string> = {
  Baja: "bg-foreground/20 text-foreground/60",
  Media: "bg-sky-500/10 border border-sky-500/20 text-sky-700",
  Alta: "bg-amber-500/10 border border-amber-500/20 text-amber-700",
  Urgente: "bg-red-500/10 border border-red-500/20 text-red-700",
};

const STATUS_STYLES: Record<TicketStatus, string> = {
  Abierto: "bg-sky-500/10 border border-sky-500/20 text-sky-700",
  "En Progreso": "bg-amber-500/10 border border-amber-500/20 text-amber-700",
  Resuelto: "bg-green-500/10 border border-green-500/20 text-green-700",
  Cerrado: "bg-foreground/20 text-foreground/50",
};
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        {projects.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-accent-strong px-4 py-2 text-xs font-bold text-white shadow-lg hover:brightness-90 transition-all outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background shrink-0"
          >
            <Plus size={14} />
  ...
  return (
    <ModalShell titleId={titleId} title="Abrir incidencia" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        {error && (
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-3 text-xs text-red-700">{error}</div>
        )}
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 rounded-xl bg-accent-strong px-4 py-3 text-xs font-bold text-white shadow-lg hover:brightness-90 transition-all disabled:opacity-50 outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        >
